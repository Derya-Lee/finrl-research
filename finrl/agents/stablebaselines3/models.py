...
class DRLAgent:
    def __init__(self, env):
        self.env = env
        # A2C handles bearish markets - PPO excels in bullish trends - DDPG is complementary, especially in mixed markets
    ...
    def DRL_prediction(model, environment, start_date, lookback_days=0):
        ...
        initial_state = {
            ...
            "daily_return": 0.0,  # TODO test fix 31 Augus -replaced reward
        ...
        while not done:
            ...
            # TODO test daily return and apply to the others:
            ...
            if not done:
                ...
                daily_record = {
                    "date": date_today,
                    "account_value": total_asset,
                    "daily_return": float(daily_returns),  # TODO test fix 31 Augus -replaced reward
                ...
                if env_inst.featured:
                    daily_record.update({
                        "volatility": env_inst.data["volatility"].iloc[0],  # rolling 30-day
