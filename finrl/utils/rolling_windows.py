...
def get_rolling_windows(train_months=TRAIN_LEN, val_months=2, trade_months=2, 
    ...
    # Define validation/trade durations as months + extra warmup days
    val_duration   = relativedelta(months=val_months)   + pd.Timedelta(days=lookback_days)
    trade_duration = relativedelta(months=trade_months) + pd.Timedelta(days=lookback_days)
